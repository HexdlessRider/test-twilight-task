...
>(({ className, checked, ...props }, ref) => (
  <CheckboxPrimitive.Root
    ...
    <CheckboxPrimitive.Indicator
      className={cn("flex items-center justify-center text-current", checked === "indeterminate" ? "bg-black" : "")}>
      {checked !== "indeterminate" ? <Check className="h-4 w-4" /> : <Minus className="h-4 w-4 text-white" />}
    </CheckboxPrimitive.Indicator>
  </CheckboxPrimitive.Root>
));
